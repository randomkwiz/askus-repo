        ...
        when (v.id) {
            R.id.btn_sign_up -> {
                ...
                if (input_password.text.toString().trim({ it <= ' ' })
                        .isEmpty() || input_repeat_password.text.toString().trim({ it <= ' ' })
                        .isEmpty() || input_email.text.toString().trim({ it <= ' ' })
                        .isEmpty() || input_nickname.text.toString().trim({ it <= ' ' })
                        .isEmpty()) {
                ...
                //Email pattern
                    if (Patterns.EMAIL_ADDRESS.matcher(
                            ...
                            .matches() && input_password.text.toString().trim(
                            { it <= ' ' }).length >= PASSWORD_MIN_LENGHT && input_password.text.toString()
                            .trim({ it <= ' ' })
